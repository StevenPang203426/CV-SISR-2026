...
用法::
    ...
    # 单张图任意倍率推理
    python -m src.liif.test --ckpt experiments/liif_edsr/best.pt --scale 3.5 \
                        --input demo/original/10.png --output results/

    # 在目录上评估 PSNR（需要 HR ground truth）
    python -m src.liif.test --ckpt experiments/liif_edsr/best.pt --scale 4 \
                        --test_dir data/DIV2K_valid_HR --output results/

    # 多倍率批量评估
    python -m src.liif.test --ckpt experiments/liif_edsr/best.pt \
...
from src.liif.liif_model import build_liif_model
from src.liif.dataset import make_coord, make_cell
...
def main():
    ...
    p.add_argument('--output', type=str, default='experiments/liif_edsr/test', help='输出目录')
