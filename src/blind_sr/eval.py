...
----
    ...
    python -m src.blind_sr.eval --experiment 1

    # 实验 2：不同退化因素的消融
    python -m src.blind_sr.eval --experiment 2

    # 实验 3：PSNR vs GAN 感知质量（在 RealSRSet 上，无参考指标）
    python -m src.blind_sr.eval --experiment 3

    # 指定 GPU
    python -m src.blind_sr.eval --experiment 1 --device cuda:0
"""
